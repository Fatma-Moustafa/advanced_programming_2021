...
class stack_pool {
    ...
    struct node_t {
        ...
        template <typename K>
        node_t(K&& value, N next, bool last) : value{ std::forward<K>(value) }, next{ next }, last{ last } {}
    ...
    */
    ...
    stack_type _push(X&& val, const stack_type head) {
        if (!std::is_same<T, value_type>::value) {
            throw DifferentType("Trying to push an invalid type.");
        }

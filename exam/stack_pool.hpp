...
/*
DiferentType, exception for pushing a diferent type of the type value
from the stack_pool. 
*/
struct DiferentType: public std::exception{
    std::string message;
    DiferentType(std::string message): message{message}{}
};
...
class stack_pool {  
    ...
    struct node_t {
        ...
        node_t(T value , N next, bool last) : value{value}, next{ next }, last{last} {}
    };
...
public:
    ...
    */
    ...
    stack_type push(const T& val, stack_type head) { 
        if(!std::is_same<T,value_type>::value){
            throw DiferentType("Trying to push an invalid type.");
            }
    ...
    */
    void print_stack(stack_type x) {
        ...
        for (auto start = begin(x), last = end(x); start != last; ++start)
        {
            std::cout <<"["<<*start << "]";
        }
        std::cout << "\n";
...
int main() {
    ...
    try{
        s1 = pool.push('B', s1); //no argument accepted
        //s2 = pool.pop(s2);
        //s2 = pool.pop(s2);
        //s2 = pool.pop(s2);
        //s2 = pool.pop(s2);
